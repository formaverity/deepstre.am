...
class AudioEngine {
  ...
  async loadBuffer(input, nameHint) {
    this.stopChord()
  ...
  // Stop every active audio node then reset volume — call after a fadeOut
  stopAll() {
    this.stopChord()
    try { if (this.grainPlayer?.state === 'started') this.grainPlayer.stop() } catch (_) {}
